...
const GroupForm = ({
  ...
  const [state, formAction] = useFormState(
    type === "create" ? createGroup : updateGroup,
    {
      success: false,
      error: false,
    }
  ...
  const { advisors, riskLevel } = relatedData;
    ...
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      ...
      <div className="flex justify-between flex-wrap gap-4">
            ...
            {advisors.map(
              (advisor: { id: string; name: string; surname: string }) => (
                <option
                  value={advisor.id}
                  key={advisor.id}
                  selected={data && advisor.id === data.supervisorId}
                >
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          >
            {riskLevel.map((riskLevel: { id: number; level: number }) => (
              <option
                value={riskLevel.id}
                key={riskLevel.id}
                selected={data && riskLevel.id === data.riskLevelId}
              >
                {riskLevel.level}
              </option>
            ))}
