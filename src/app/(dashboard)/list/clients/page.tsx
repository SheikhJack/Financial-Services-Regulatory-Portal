...
import { Group, Prisma, Client } from "@prisma/client";
import Image from "next/image";
import Link from "next/link";

import { auth } from "@clerk/nextjs/server";

type ClientList = Client & { group: Group };
...
const ClientListPage = async ({
  ...
  const renderRow = (item: ClientList) => (
    <tr
      key={item.id}
    ...
    >
      <td className="flex items-center gap-4 p-4">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-xs text-gray-500">{item.group.name}</p>
        </div>
      </td>
      <td className="hidden md:table-cell">{item.username}</td>
      <td className="hidden md:table-cell">{item.group.name[0]}</td>
