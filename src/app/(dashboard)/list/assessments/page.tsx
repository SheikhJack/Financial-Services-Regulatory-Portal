...
import { Assessment, Advisor, Regulation, Group } from "@prisma/client";
import Image from "next/image";
import { auth } from "@clerk/nextjs/server";

// Define exam type with nested relations
...
const AssessmentListPage = async ({
  ...
  const columns = [
    { header: "Regulation", accessor: "name" },
    { header: "Group", accessor: "class" },
    {
      header: "Advisor",
      accessor: "advisor",
  ...
  const renderRow = (item: AssessmentList) => (
    ...
    >
      <td className="flex items-center gap-4 p-4">
        {item.resource.regulation.name}
      </td>
      <td>{item.resource.group.name}</td>
